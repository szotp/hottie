...
Future<Uri> generateMain(TestFileHandles testPaths, {bool overrideMain = false}) async {
  final hottieImport = await _prepareHottieInsiderImport();
...
Future<String> _prepareHottieInsiderImport() async {
  bool importsHottie() {
    // parse package config and check if hottie is available. if yes, return normal import
    final packageConfigFile = File('pubspec.yaml');

    if (!packageConfigFile.existsSync()) {
      return false;
    }

    final string = packageConfigFile.readAsStringSync();
    return string.contains('hottie:');
  }
